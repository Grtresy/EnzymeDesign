...
def run(
    ...
    if mode == "mock":
        write_json(docking_path, build_docking("vina"))
        return ToolRunResult(outputs={"docking": str(docking_path)}, command=None)

    executable = require_executable("vina")
    command = [
        executable,
        "--receptor",
        str(receptor_path),
        ...
        str(ligand_path),
        "--out",
        str(output_path / "docked.pdbqt"),
    ]
    if not docking_path.exists():
        raise RuntimeError(
            "Expected AutoDock Vina output not found. Ensure vina ran successfully."
        )
    write_json(docking_path, build_docking("vina"))
