...
def get_version(mode: str) -> str:
    return "mock-af3-1.0" if mode == "mock" else "real-af3-1.0"
...
def run(
    ...
    output_path = outputs["af3_metrics"]
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(["candidate_id", "pLDDT", "pTM", "active_site_rmsd"])
        for candidate_id, _seq in records:
            writer.writerow(
                [
                    candidate_id,
                    round(rng.uniform(70.0, 95.0), 2),
                    round(rng.uniform(0.6, 0.9), 2),
                    round(rng.uniform(0.8, 2.5), 2),
                ]
            )
