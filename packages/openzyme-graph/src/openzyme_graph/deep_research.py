...
from openzyme_runtime import ResearchSupervisorAction
...
def _research_turns(inputs: GraphAssemblyInputs, episode_id: str, limit: int = 10) -> list[ResearchTurnRecord]:
    turns = [
        turn
        for turn in inputs.repositories.decisions.list_by_episode(episode_id)
        if turn.phase == RESEARCH_PHASE
    ]
    return [
        ResearchTurnRecord(
            ...
            tool_names=[] if turn.action_payload is None else list(turn.action_payload.get("tool_names") or []),
            observation_summary=None
            if turn.observation_payload is None
            else (
...
def _record_research_turn(
    ...
    *,
    ...
    current_turns = [
        item for item in inputs.repositories.decisions.list_by_episode(episode_id) if item.phase == RESEARCH_PHASE
    ]
    turn_index = len(current_turns) + 1
    created_at = _utc_now_iso()
    inputs.repositories.decisions.save(
        Decision(
            decision_id=f"{episode_id}-research-turn-{turn_index}",
            ...
            action_payload=None if tool_names is None else {"tool_names": tool_names},
    ...
    return ResearchTurnRecord(
        ...
        if observation_payload is None
            ...
            observation_payload.get("summary")
            or observation_payload.get("message")
        ),
        created_at=created_at,
    )
...
def _summarize_unit_observations(observations: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[str], list[str], str]:
    ...
    return findings, unresolved_gaps, summary_parts, summary
...
def _run_research_unit(
    ...
    findings, unresolved_gaps, _summary_parts, summary = _summarize_unit_observations(observations)
    ...
    def dispatch_research_units(
        ...
        if max_workers == 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                unit_results = list(
                    executor.map(
                        lambda unit: _run_research_unit(inputs, state, unit),
                        scheduled_units,
                    )
                )
    ...
    def synthesize_research_dossier(state: DeepResearchState) -> dict[str, Any]:
        if state.get("clarification_question"):
            dossier = ResearchDossier(
                ...
                raw_notes=list(state.get("raw_notes") or []),
                recent_turns=_research_turns(inputs, str(state["episode_id"])),
            )
