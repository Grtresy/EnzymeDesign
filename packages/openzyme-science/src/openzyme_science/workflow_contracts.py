...
def _canonical_workflow_digest(value: object) -> str:
    """Preserve the already-public workflow contract digest byte convention."""
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "operation_signatures": [
                signature.to_dict() for signature in sorted(self.operation_signatures)
            ],
...
class ScientificWorkflowContract:
    ...
    def __post_init__(self) -> None:
        ...
        if self.effect_adoption_policy != SCIENTIFIC_EFFECT_ADOPTION_POLICY_ATOMIC:
            raise ValueError("unsupported scientific effect adoption policy")
        if self.same_attempt_reuse_policy != SCIENTIFIC_SAME_ATTEMPT_REUSE_POLICY:
            raise ValueError("unsupported scientific same-attempt reuse policy")
        _require_identifier(
    ...
    @property
    def digest(self) -> str:
        return _canonical_workflow_digest(self.canonical_preimage)
        ...
        if policy is None:
            raise ScientificWorkflowContractError(
                ...
                details={
                    ...
                    "allowed_scopes": sorted(item.scope.value for item in self.scopes),
                },
            )
        return policy
