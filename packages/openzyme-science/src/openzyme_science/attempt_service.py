...
from openzyme_contracts import ControlledOperation
from openzyme_contracts import ControlledOperationExecution
...
from .attempts import ScientificAttemptAdmissionRequest
from .attempts import ScientificAttemptAuthorization
from .attempts import ScientificAttemptAuthorityStatus
from .attempts import ScientificAttemptClosure
from .attempts import ScientificAttemptClosureRequest
from .attempts import ScientificAttemptScope
from .attempts import ScientificAttemptStatus
from .attempts import ScientificChainSelection
from .attempts import ScientificEffectAdoption
from .attempts import ScientificOperationDisposition
from .attempts import ScientificOperationDispositionKind
from .attempts import ScientificSelectionState
from .selection_records import ResolvedScientificSelectionHead
from .selection_records import ScientificAttemptIdentityConflictError
from .selection_records import ScientificOccurrenceSnapshot
from .selection_records import ScientificSelectionIntegrityError
from .attempt_lifecycle import ResolvedScientificAttemptLifecycle
...
from .attempt_rollover import scientific_attempt_post_scope_id
from .attempt_rollover import scientific_attempt_post_scope_ref
from .selection_evaluation import ScientificSelectionEvaluation
from .selection_evaluation import ScientificSelectionEvaluator
from .workflow_contracts import (
    SCIENTIFIC_EFFECT_ADOPTION_POLICY_ATOMIC,
)
from .workflow_contracts import HistoricalScientificWorkflowContract
from .workflow_contracts import ScientificWorkflowContract
from .workflow_contracts import ScientificWorkflowContractError
from .workflow_contracts import ScientificWorkflowContractRegistry
...
def canonical_digest(value: object) -> str:
    """Preserve the existing Science control-plane digest byte convention."""
...
class ScientificAttemptRepositoryView(Protocol):
    """Transitional repository view while writes move to the restricted participant.

    The view intentionally has no Core type dependency.  Its broad legacy fields are
    confined to the migration adapter and are removed by task 12.4.
    """

    scientific_attempt_authorizations: Any
    scientific_attempt_admission_requests: Any
    scientific_attempts: Any
    scientific_attempt_bindings: Any
    scientific_attempt_closure_requests: Any
    scientific_attempt_closures: Any
    scientific_selections: Any
    scientific_dispositions: Any
    scientific_effect_adoptions: Any
    controlled_operations: Any
    controlled_operation_executions: Any
    controlled_operation_results: Any
    continuation_states: Any
    mutation_scopes: Any
    mutation_writers: Any
    quiescence_receipts: Any
    quiescence_snapshots: Any
    lanes: Any
    tasks: Any
    sandbox_runs: Any

    def atomic(self, *, prefix: str) -> AbstractContextManager[None]: ...


class ScientificMutationScopeApplication(Protocol):
    def writer_turn(self, **kwargs: Any) -> AbstractContextManager[object]: ...

    def open_scope(self, **kwargs: Any) -> Any: ...

    def begin_freeze(self, scope_id: str) -> Any: ...

    def issue_quiescence_receipt(self, scope_id: str) -> Any: ...

    def seal_scope(self, *args: Any, **kwargs: Any) -> Any: ...


class ScientificQuiescenceApplication(Protocol):
    def verify(self, *, receipt: object, snapshot: object) -> None: ...

    def build_envelope(self, *, receipt: object, snapshot: object) -> dict[str, Any]: ...


class ScientificCampaignUnknownEffectReader(Protocol):
    """Migration seam for one bounded Core controlled-effect query."""
...
ScientificMutationScopeFactory = Callable[
    [ScientificAttemptRepositoryView, Callable[[], str], Callable[[], str]],
    ScientificMutationScopeApplication,
]
...
class ScientificAttemptService:
    repositories: ScientificAttemptRepositoryView
    mutation_scope_factory: ScientificMutationScopeFactory
    current_mutation_authority: Callable[[], object | None]
    quiescence: ScientificQuiescenceApplication
    campaign_unknown_effects: ScientificCampaignUnknownEffectReader
    now: Callable[[], str] = _utc_now_iso
    id_factory: Callable[[], str] = lambda: uuid4().hex
    workflow_contract_registry: ScientificWorkflowContractRegistry | None = None

    @property
    def mutation_scopes(self) -> ScientificMutationScopeApplication:
        return self.mutation_scope_factory(
            self.repositories,
            self.now,
            self.id_factory,
        )
    ...
    def finalize_attempt_admission(
        ...
        *,
        ...
        if self.current_mutation_authority() is not None:
            raise ScientificAttemptError(
    ...
    def close_attempt(
        ...
        *,
        ...
        self.quiescence.verify(receipt=receipt, snapshot=snapshot)
        authority = self.repositories.scientific_attempt_authorizations.get(
            attempt.envelope_id
        )
        if authority is None:
    ...
    def export_closed_attempt_evidence(
        ...
        *,
        ...
        payload: dict[str, Any] = {
            ...
            "quiescence": self.quiescence.build_envelope(
                receipt=receipt,
                snapshot=snapshot,
            ),
    ...
    def _assert_no_campaign_unknown_effect(
        ...
        *,
        ...
        operation_ids = self.campaign_unknown_effects.list_operation_ids(
            session_id=session_id,
            task_id=task_id,
            campaign_id=campaign_id,
        )
...
__all__ = [
    ...
    "ScientificAttemptError",
    "ScientificCampaignUnknownEffectReader",
    "ScientificAttemptRepositoryView",
    "ScientificAttemptService",
    "ScientificMutationScopeApplication",
    "ScientificMutationScopeFactory",
    "ScientificOperationAdoptionResult",
    "ScientificOperationUniverse",
    "ScientificQuiescenceApplication",
    "canonical_digest",
    "scientific_attempt_authorization_identity",
