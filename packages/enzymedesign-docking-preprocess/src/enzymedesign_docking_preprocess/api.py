...
class PreprocessError(RuntimeError):
    """A local docking preprocessing operation failed explicitly."""
    ...
    *,
    ...
    if executable is None:
        raise PreprocessError(
            "OpenBabel executable 'obabel' is required for this conversion"
        )
    ...
    if result.returncode != 0:
        message = (
            result.stderr or result.stdout or "OpenBabel conversion failed"
        ).strip()
