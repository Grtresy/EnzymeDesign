...
def load_agent_capsule_image_manifest() -> AgentCapsuleImageManifest:
    resource = files("openzyme_process_podman.agent_capsule_assets").joinpath(
        "manifest.json"
    )
...
def build_agent_capsule_image(
    *,
    ...
    asset_root = files("openzyme_process_podman.agent_capsule_assets")
    package_root = Path(__file__).resolve().parents[3]
    sdk_sources = {
        "openzyme_execution_sdk": (
            package_root / "openzyme-execution-sdk" / "src" / "openzyme_execution_sdk"
        ),
    ...
    missing = sorted(
        namespace for namespace, source in sdk_sources.items() if not source.is_dir()
    )
    if missing:
        raise AgentCapsuleImageError(
            "agent capsule build requires exact monorepo execution SDK sources: "
            + ", ".join(missing)
        )
    with as_file(asset_root) as asset_path, tempfile.TemporaryDirectory(
        ...
        for namespace, source in sdk_sources.items():
            shutil.copytree(
                source,
                context_path / "execution_sdk" / namespace,
