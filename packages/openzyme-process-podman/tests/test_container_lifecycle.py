...
from openzyme_process_podman import PodmanContainerLease
...
def test_container_lease_retires_exact_cid_after_transient_faults_and_name_drift(
    ...
    monkeypatch.setattr("openzyme_process_podman.lifecycle.subprocess.run", fake_run)
    monkeypatch.setattr("openzyme_process_podman.lifecycle.os.read", transient_cid_read)
    monkeypatch.setattr(
        "openzyme_process_podman.lifecycle.Path.unlink",
