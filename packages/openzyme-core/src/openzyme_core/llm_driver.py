...
def _build_system_prompt(context: SessionRuntimeContext) -> str:
    ...
    sections = [
        ...
        "Use world.inspect when you need structured facts about tasks, workspace revisions, publications, approvals, operations, outcomes, runtime warnings, visible tools, or route policies; it is an observation tool, not a planner.",
        ...
        "Delegate file-producing work to an agent with a generation-owned clone. Ordinary file work uses workspace.exec and native Git; sharing requires an explicit immutable publication or typed revision/path handoff. Never request Host-local or runner-private paths.",
