...
INVENTORY_SCHEMA = "device_fresh_install_reset_inventory@1"
OCCURRENCE_SCHEMA = "device_fresh_install_deletion_occurrence@1"
PERMISSION_ADJUSTMENT_SCHEMA = "device_fresh_install_permission_adjustment@1"
RECEIPT_SCHEMA = "device_fresh_install_reset_receipt@1"
...
def _parse_plan(
    ...
    for target in targets:
        if not target.target_kind or not target.owner_evidence:
            raise DeviceFreshResetError(
                "reset_target_owner_missing",
                "target lacks kind or OpenZyme owner evidence",
                phase="plan_validation",
                identity=str(target.path),
                expected="non-empty target_kind and owner_evidence",
                observed={
        ...
        if occurrence is not None:
            ...
            if exists:
                ...
                replacement_matches = (
                    ...
                    and current["inode"] != expected.get("inode")
                )
