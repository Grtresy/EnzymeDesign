...
def test_owner_local_runner_source_uses_matching_test_and_frontend_omission() -> None:
    scope_map = load_affected_scope_map(MAP_PATH, repo_root=REPOSITORY_ROOT)

    selection = expand_affected_scope(
        REPOSITORY_ROOT,
        inventory=_inventory(
            "apps/mcp-hpc-runner/src/mcp_hpc_runner/server.py"
        ),
        scope_map=scope_map,
    )

    assert selection.matched_rules == ("mcp-hpc-runner-owner",)
    assert selection.pytest_paths == ("apps/mcp-hpc-runner/tests",)
    assert selection.lint_paths == ("apps/mcp-hpc-runner",)
...
def test_host_and_ui_changes_include_frontend_contracts() -> None:
    scope_map = load_affected_scope_map(MAP_PATH, repo_root=REPOSITORY_ROOT)

    host = expand_affected_scope(
        REPOSITORY_ROOT,
        inventory=_inventory("apps/openzyme-host-api/src/openzyme_host_api/v2_app.py"),
        scope_map=scope_map,
    )
    ...
    assert host.matched_rules == ("host-api-owner-and-ui-contract",)
    assert host.frontend is True
    assert host.pytest_paths == ("apps/openzyme-host-api/tests/test_v2_app.py",)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "packages/openzyme-kernel/src/openzyme_kernel/projection.py",
    ),
)
...
@pytest.mark.parametrize(
    "changed_path",
    (
        "packages/openzyme-standard/pyproject.toml",
