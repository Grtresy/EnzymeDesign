...
def test_canonical_cancellation_receipt_round_trips_through_domain() -> None:
    ...
    wrapped = WorkspaceJobCancellationReceipt(
        receipt_id=str(receipt["receipt_id"]),
        cancellation_id=str(receipt["cancellation_id"]),
        handle_id=str(receipt["handle_id"]),
        cancellation_requested=True,
        terminal_settlement_proven=False,
        backend_receipt_digest=str(receipt["backend_receipt_digest"]),
        created_at=str(receipt["created_at"]),
        receipt_digest=str(receipt["receipt_digest"]),
    )
    assert wrapped.payload["receipt_id"] == "cancel_receipt_1"
    assert wrapped.receipt_digest == receipt["receipt_digest"]
...
def test_domain_enum_values_use_the_same_canonical_digest() -> None:
    left = canonical_workspace_job_wire_digest({"backend": WorkspaceExternalBackend.DIRECT})
    right = canonical_workspace_job_wire_digest({"backend": "direct"})
    assert left == right
