...
class ScientificFileEffectAdoption:
    ...
    def __post_init__(self) -> None:
        ...
        if self.adoption_digest != canonical_scientific_deliverable_digest(self.payload):
...
class ScientificDeliverableRef:
    ...
    ref_digest: str
    schema_version: str = "scientific_deliverable_ref@1"

    def __post_init__(self) -> None:
        if self.schema_version != "scientific_deliverable_ref@1":
        ...
        if not self.publication_ref.startswith("refs/openzyme/publications/"):
            raise ValueError("scientific deliverable requires an immutable publication ref")
        if self.storage is ScientificFileStorage.GIT_BLOB:
            if (
                ...
                raise ValueError("ordinary Git deliverable has inconsistent byte identity")
            _require_oid("git_blob_oid", self.git_blob_oid)
        else:
            if self.git_blob_oid is not None or self.lfs_oid is None:
                raise ValueError("Git LFS deliverable has inconsistent pointer identity")
