...
class StandardRuntimeDrainApplication(Protocol):
...
class StandardWorkspaceToolRuntime(Protocol):
...
class StandardKernelOperationalRouteApplication:
    ...
    implementation.  Standard selects those Adapters and injects the already-mounted
    ...
    workspace_tools: Mapping[str, StandardWorkspaceToolRuntime]
    ...
    def __post_init__(self) -> None:
        ...
        if set(self.workspace_tools) != expected:
            raise ValueError(
                "Standard operational HTTP routes require exact workspace mutation "
    ...
    def _workspace_tool(
        ...
        return KernelMutationReceipt.create(
            ...
            service_id="openzyme.standard.workspace-route",
            operation=tool_name,
...
def build_standard_operational_route_applications(
    application: StandardKernelOperationalRouteApplication,
) -> dict[str, StandardKernelOperationalRouteApplication]:
...
def _payload_error(route_id: str, message: str) -> HostV2CommandError:
    return HostV2CommandError(
        "standard_operational_route_payload_invalid",
        message,
        status_code=422,
