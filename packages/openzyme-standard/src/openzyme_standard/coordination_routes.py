...
class StandardKernelCoordinationRouteApplication:
    ...
    def invoke(self, invocation: HostV2MutationInvocation) -> KernelMutationReceipt:
        ...
        raise HostV2CommandError(
            "standard_coordination_route_unknown",
...
def build_standard_coordination_route_applications(
    application: StandardKernelCoordinationRouteApplication,
) -> dict[str, StandardKernelCoordinationRouteApplication]:
    return {route_id: application for route_id in STANDARD_COORDINATION_ROUTE_IDS}


def build_standard_command_context(
    ...
    *,
    ...
    if invocation.actor_id.startswith("agent-member:"):
        ...
        raise HostV2CommandError(
            "standard_operator_agent_authority_required",
...
def _message_ingress_context(
    ...
    *,
    ...
    if not invocation.actor_id.startswith("user:"):
        return build_standard_command_context(invocation, ids=ids)
    core = invocation.precondition.projection.core.payload
    agents = core.get("agents")
    ...
    return build_standard_command_context(projected, ids=ids)
...
def _projection_error(message: str) -> HostV2CommandError:
    return HostV2CommandError(
        "standard_kernel_route_projection_stale",
        message,
        status_code=409,
        mutation_applied=False,
        effect_certainty="no_effect",
