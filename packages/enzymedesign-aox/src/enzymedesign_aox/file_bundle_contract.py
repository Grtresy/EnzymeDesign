...
class AoxScientificFileContractError(RuntimeError):
    error_code = "aox_scientific_file_contract_rejected"
...
AOX_SCIENTIFIC_FILE_ROLES: tuple[AoxScientificFileRole, ...] = (
    ...
    AoxScientificFileRole(
        "reference_panel_fasta", "aox_hmm/AOX_ref21.fasta", "fasta@1"
    ),
    AoxScientificFileRole(
        "scoring_input_fasta", "aox_hmm/AOX_scoring_input.fasta", "fasta@1"
    ),
    AoxScientificFileRole(
...
def aox_format_contract_digest(format_contract_id: str) -> str:
    if format_contract_id not in {
        ...
        raise AoxScientificFileContractError(
...
def validate_aox_scientific_file_bytes(
    ...
    if not isinstance(summary, dict):
        raise AoxScientificFileContractError("AOX execution summary requires an object")
    ...
    for path, text in texts.items():
        ...
        if format_contract_id == "csv@1":
            ...
            if (
                ...
                or empty_file.get("schema_id") != AOX_CONDITIONAL_EMPTY_FILE_SCHEMA_ID
    ...
    return {
        ...
        + hashlib.sha256(
            b"".join(
                path.encode("utf-8") + b"\0" + hashlib.sha256(files[path]).digest()
...
__all__ = [
    ...
    "AoxScientificFileContractError",
    "AoxScientificFileRole",
