...
from openzyme_science import HistoricalScientificWorkflowContract
...
from openzyme_science import ScientificWorkflowContract
from openzyme_science import ScientificWorkflowContractRegistry
from openzyme_science import ScientificWorkflowRolePolicy
from openzyme_science import ScientificWorkflowScopePolicy


AOX_SELECTED_CHAIN_WORKFLOW_ID: Final = "aox_blank_world"
AOX_SELECTED_CHAIN_WORKFLOW_CONTRACT_V1_ID: Final = "aox_blank_world_selected_chain@1"
AOX_SELECTED_CHAIN_WORKFLOW_CONTRACT_ID: Final = "aox_blank_world_selected_chain@2"
...
def _historical_canonical_digest(value: object) -> str:
    """Preserve the frozen pre-Contracts canonical JSON byte convention."""
...
AOX_SELECTED_CHAIN_WORKFLOW_CONTRACT_V1_DIGEST: Final = _historical_canonical_digest(
...
AOX_SELECTED_CHAIN_CONTRACT_V2: Final = ScientificWorkflowContract(
    ...
    scopes=(
        ScientificWorkflowScopePolicy(
            ...
            roles=tuple(
                _role_policy(role) for role in sorted(AOX_FORMAL_WORKFLOW_ROLES)
            ),
        ),
        ScientificWorkflowScopePolicy(
        ...
        ScientificWorkflowScopePolicy(
            ...
            roles=tuple(
                _role_policy(role) for role in sorted(AOX_PROBE_WORKFLOW_ROLES)
            ),
        ),
    ),
...
AOX_SELECTED_CHAIN_CONTRACT_V1_READER: Final = HistoricalScientificWorkflowContract(
    schema_id=str(AOX_SELECTED_CHAIN_WORKFLOW_CONTRACT_V1["schema_id"]),
    contract_id=AOX_SELECTED_CHAIN_WORKFLOW_CONTRACT_V1_ID,
    workflow_id=AOX_SELECTED_CHAIN_WORKFLOW_ID,
...
AOX_SCIENTIFIC_WORKFLOW_CONTRACT_REGISTRY: Final = ScientificWorkflowContractRegistry(
