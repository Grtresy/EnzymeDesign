...
class RepositoryServicePreflightError(RuntimeError):
    """Fail-closed startup or dynamic qualification failure."""
...
class RepositoryTransportSettings(Protocol):
    bare_repository_root: Path
    git_http_backend: Path


class RepositoryServicePreflightReport(Protocol):
    pre_receive_hook_digest: str
    active_bindings: tuple[dict[str, object], ...]
    inventory_digest: str


class RepositoryAggregate(Protocol):
...
class RepositoryProvider(Protocol):
    def read(self) -> AbstractContextManager[RepositoryScope]: ...

    def write(self) -> AbstractContextManager[RepositoryScope]: ...


class RepositoryTransportDependencies(Protocol):
    """Narrow composition boundary consumed by the transport mechanism."""

    repository_provider: RepositoryProvider
    settings: RepositoryTransportSettings
...
def _authenticate_request(
    ...
    *,
    ...
    """Rejoin Host authority admission with Adapter-owned storage identity."""
    ...
    dependencies.roots().verify_bare_repository(authenticated.binding)
