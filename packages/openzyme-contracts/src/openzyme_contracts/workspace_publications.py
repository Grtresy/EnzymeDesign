...
class PublicationManifestEntry:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "PublicationManifestEntry":
        expected = {
            "path",
            "mode",
            "object_kind",
        ...
        if set(payload) != expected:
            raise ValueError("publication manifest entry has an invalid closed schema")
        for key in ("path", "mode", "object_kind", "object_id"):
            if not isinstance(payload[key], str):
                raise TypeError(f"{key} must be a string")
        for key in ("size_bytes", "lfs_size_bytes"):
        ...
        if payload["lfs_oid"] is not None and not isinstance(payload["lfs_oid"], str):
            raise TypeError("lfs_oid must be a string or null")
        return cls(
            path=payload["path"],
            mode=payload["mode"],
            object_kind=PublicationManifestObjectKind(payload["object_kind"]),
            object_id=payload["object_id"],
            size_bytes=payload["size_bytes"],
            lfs_oid=payload["lfs_oid"],
            lfs_size_bytes=payload["lfs_size_bytes"],
        )
...
class WorkspacePublicationManifest:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "WorkspacePublicationManifest":
        if set(payload) != {"schema_version", "entries", "manifest_digest"}:
            raise ValueError("workspace publication manifest has an invalid closed schema")
        if not isinstance(payload["schema_version"], str):
            raise TypeError("schema_version must be a string")
        if not isinstance(payload["manifest_digest"], str):
            raise TypeError("manifest_digest must be a string")
        entries = payload["entries"]
        if not isinstance(entries, list | tuple) or not all(
            isinstance(item, Mapping) for item in entries
        ):
            raise TypeError("manifest entries must be an array of objects")
        return cls(
            entries=tuple(
                PublicationManifestEntry.from_dict(dict(item)) for item in entries
            ),
...
class WorkspacePublicationIntent:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "WorkspacePublicationIntent":
        expected = set(cls.__dataclass_fields__)  # type: ignore[attr-defined]
        if set(payload) != expected:
            raise ValueError("workspace publication intent has an invalid closed schema")
        manifest = payload["manifest"]
        if not isinstance(manifest, Mapping):
            raise TypeError("manifest must be an object")
        parents = payload["git_parent_commits"]
        if not isinstance(parents, list | tuple) or not all(
            isinstance(item, str) for item in parents
        ):
            raise TypeError("git_parent_commits must be an array of strings")
        values = dict(payload)
        values["manifest"] = WorkspacePublicationManifest.from_dict(dict(manifest))
        values["git_parent_commits"] = tuple(parents)
        values["state"] = WorkspacePublicationIntentState(payload["state"])
        return cls(**values)
...
class WorkspacePublicationRemoteReceipt:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "WorkspacePublicationRemoteReceipt":
        expected = set(cls.__dataclass_fields__)  # type: ignore[attr-defined]
        if set(payload) != expected:
            raise ValueError("publication remote receipt has an invalid closed schema")
        return cls(**payload)
...
class PublishedRevision:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "PublishedRevision":
        expected = set(cls.__dataclass_fields__)  # type: ignore[attr-defined]
        if set(payload) != expected:
            raise ValueError("published revision has an invalid closed schema")
        manifest = payload["manifest"]
        parents = payload["git_parent_commits"]
        if not isinstance(manifest, Mapping):
            raise TypeError("manifest must be an object")
