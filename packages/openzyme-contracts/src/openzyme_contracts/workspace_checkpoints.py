...
class RemotePrivateRefObservation:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "RemotePrivateRefObservation":
        expected = {
            "schema_version",
            "service_id",
            "repository_id",
        ...
        if set(payload) != expected:
            raise ValueError("remote private ref observation has an invalid closed schema")
        prior = payload["prior_commit"]
        if prior is not None and not isinstance(prior, str):
            raise TypeError("prior_commit must be a string or null")
        for key in expected - {"prior_commit"}:
        ...
        return cls(
            ...
            advance_kind=PrivateRefAdvanceKind(payload["advance_kind"]),
...
class WorkspaceCheckpointProofInput:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "tree": self.tree,
            "private_ref": self.private_ref,
            "remote_observation": self.remote_observation.to_dict(),
        }

    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "WorkspaceCheckpointProofInput":
        expected = {
            "schema_version",
            "boundary",
            "workspace_id",
        ...
        if set(payload) != expected:
            raise ValueError("workspace checkpoint proof input has an invalid closed schema")
        for key in expected - {
            "workspace_generation",
            "repository_binding_version",
            "remote_observation",
            ...
            if not isinstance(payload[key], str):
                raise TypeError(f"{key} must be a string")
        for key in ("workspace_generation", "repository_binding_version"):
            if not isinstance(payload[key], int) or isinstance(payload[key], bool):
                raise TypeError(f"{key} must be an integer")
        observation = payload["remote_observation"]
        if not isinstance(observation, Mapping):
            raise TypeError("remote_observation must be an object")
        return cls(
            boundary=WorkspaceFormalBoundary(payload["boundary"]),
            ...
            repository_binding_version=payload["repository_binding_version"],
            commit=payload["commit"],
            tree=payload["tree"],
            private_ref=payload["private_ref"],
            remote_observation=RemotePrivateRefObservation.from_dict(dict(observation)),
            schema_version=payload["schema_version"],
        )
...
class VerifiedWorkspaceCheckpoint:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {**self.identity_payload, "checkpoint_digest": self.checkpoint_digest}

    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "VerifiedWorkspaceCheckpoint":
        expected = {
            "schema_version",
            "checkpoint_id",
            "boundary",
        ...
        if set(payload) != expected:
            raise ValueError("verified workspace checkpoint has an invalid closed schema")
        for key in expected - {
            "workspace_generation",
            "repository_binding_version",
            "prior_commit",
        ...
        return cls(
            ...
            prior_commit=prior,
            advance_kind=PrivateRefAdvanceKind(payload["advance_kind"]),
            remote_observed_at=payload["remote_observed_at"],
