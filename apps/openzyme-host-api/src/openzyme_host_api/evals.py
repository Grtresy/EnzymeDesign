...
from .aox_final_deliverable_validation import (
    S15_AOX_HMM_OLD_DELIVERABLES as _S15_AOX_HMM_OLD_DELIVERABLES,
)
from .aox_final_deliverable_validation import (
    aox_legacy_paths_present as _s15_aox_legacy_paths_present,
)
from .aox_final_deliverable_validation import (
    aox_required_artifact_paths as _s15_aox_required_artifact_paths,
)
from .aox_final_deliverable_validation import (
    validate_aox_final_artifacts as _s15_aox_validate_final_artifacts,
)
...
S15_AOX_HMM_FIXED_PROMPT = (
    ...
    "integer-tenths threshold 336 (display 33.6), derive AOX_candidates.fasta only with "
    "aox_motif_candidate_filter@1 from canonical passes_motif_rule rows, and deduplicate "
    "at similarity threshold 0.85. Conditional-empty outputs must come only from the exact "
    "typed aox_upstream_empty_materialization@1, "
    "aox_reference_only_scoring_alignment@1, or aox_empty_membership@1 calculation for the "
    "observed branch. Prepare, but do not individually register, the exact 17 normalized "
    "deliverables, then call aox_final_deliverable_bundle@1 once so the Host prevalidates "
    "and atomically catalogs the whole set with one source-bound "
    "aox_final_deliverable_validation_receipt@1. Attempt closure, execution completion, "
    "report handoff, publication, and report completion must all be gated on that exact "
    "receipt. The normalized set under aox_hmm/ is: AOX_ref21.fasta, "
