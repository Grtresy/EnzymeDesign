...
ATTEMPT_PREFLIGHT_SCHEMA_ID = "aox_attempt_preflight@1"
ATTEMPT_PREFLIGHT_FILENAME = "aox-attempt-preflight.json"
_PREFLIGHT_FIELDS = {
    "schema_id", "run_class", "campaign_id", "plan_digest",
    "consumption_digest", "identity_digest", "allowed_prerequisite_digest",
    "architecture_qualification_digest", "effective_config", "slot",
    "root_proof", "created_at", "receipt_digest",
}
_ROOT_PROOF_FIELDS = {
...
def build_attempt_preflight_receipt(
    *,
    ...
    if not all((
        ...
        authority_consumption.get("plan_digest") == authority_plan.get("plan_digest"),
        ...
        _reject("attempt_preflight_source_mismatch", "preflight sources do not bind one slot")
...
def load_attempt_preflight_receipt(
    ...
    slot, proof = value.get("slot"), value.get("root_proof")
    ...
    if not structural:
        _reject("attempt_preflight_invalid", "preflight receipt is unsafe or noncanonical")
    assert isinstance(slot, dict) and isinstance(proof, dict)
    proof_valid = all((
    ...
    if require_unstarted:
        ...
        if (attempt_root / "control-plane.sqlite3").exists() or nonempty or (
            evidence_entries != [ATTEMPT_PREFLIGHT_FILENAME]
        ):
...
__all__ = [
    "ATTEMPT_PREFLIGHT_FILENAME", "ATTEMPT_PREFLIGHT_SCHEMA_ID",
    "build_attempt_preflight_receipt", "load_attempt_preflight_receipt",
