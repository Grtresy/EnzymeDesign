...
class LiveAoxAttemptRunner:
    ...
    def _formal_prompt(self, context: AttemptRunContext) -> str:
        ...
        prompt = (
            ...
            + "closure_request_ready means the sealed selection can support master-authored "
            + "closure intent in the current turn; closure_finalization_ready and the legacy "
            ...
            + "an unavailable capability: after sealing a positive selection, the executor must "
            + "finish its canonical task completed with the actual result evidence and leave "
            + "report publication plus closure to the reporter and resident master. The runtime "
            + "rejects a blocked/failed/cancelled positive execution exit after that seal. "
