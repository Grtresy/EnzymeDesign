# enzymedesign-external-route-qualification Specification

## Purpose
定义 EnzymeDesign 真实外部 route 的 source-bound identity discovery、operator decision、preparation、dry plan、一次性 authority、预算、effect、证据、TTL 与 target helper deployment 边界。
...
Batch qualification MAY aggregate current receipts from multiple one-shot occurrences only when every receipt binds the same exact checkout source identity, dry plan and current unit/subject/route/schema closure. The checkout source identity MUST be persisted with the occurrence subset before effect and MUST NOT be inferred from an unchanged dry-plan digest. An independent receipt-set verifier MUST validate each receipt's authorization, persisted source and occurrence subset, negative gate, budget settlement, cleanup evidence and validity interval, select at most one deterministic current receipt per required unit, and leave every missing or rejected unit blocked. A successful subset occurrence MUST NOT itself emit a batch-level `qualified` claim.
