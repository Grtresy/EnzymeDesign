...
- [x] 4.5 实现response-loss reconciliation，仅查询同一preallocated ref：exact commit收敛原execution/publication，different commit形成integrity conflict，无法证明则保持`dispatch_in_doubt`。
...
- [x] 4.8 增加execution/recovery focused tests，覆盖success receipt、response loss exact-match、different-ref conflict、unreconcilable unknown、stale fence、dispatch前lease revoke为`no_effect`、possible-effect后revoke仍same-ref reconcile、restart materialization和零replacement dispatch。
