...
- [x] 18.10 在exact clean final source上生成completion evidence，确认代码、配置、migration、OpenSpec、文档、wheels和qualification同一identity后才允许archive；不得因artifacts apply-ready、旧receipt或单一绿色gate提前完成。clean candidate `ba10837dc233a09617d0b1b3da18a481f5ad709d` 已通过 37-wheel 闭包、正式 admission、独立 verifier、authoritative mainline、eval 与 strict OpenSpec；本证据封口进入同一逻辑提交后，最终 clean SHA 必须整组复跑，最终 SHA/报告/digest 仅记录在 checkout 外的交付说明中，避免仓内证据自引用。第二次 reset receipt 保持独立有效且本轮未重置数据库。
...
- [x] 19.5 为 Compute 和需要 durable extension state 的产品路径选择并实现 SQLite-backed repositories/UoW 绑定，证明 Host restart 后 invocation、continuation、route identity 与 result settlement 可恢复且 Plugin 不直接写 Core tables。Compute record 现经 `ExtensionStateKernelApplicationService`、`ComputeTransactionParticipant` 与 Store-owned namespaced table 做 CAS 持久化；EnzymeDesign composition root 构造 exact Session pin/binding reader、SQLite coordinator/query、durable Compute repository 和 Kernel continuation service。真实 SQLite restart 测试证明 state version 2 的原 occurrence 不 redispatch、原 opaque handle 可结算到 version 3、第二次重启仍可恢复 result 与 continuation；同时修复了 continuation `source_version` 从兼容 TEXT 列读回时的类型/digest 漂移。
...
- [x] 19.8 建立真实 `enzymedesign_local_single_process_file_sqlite@1` 非 live 产品场景：从 generic Host、Session composition pin、authority、PublishedRevision、adopted inventory、affordance/route 到 HMMER/Vina、Compute、声明式 fake external runner、result validation、owner continuation 和 Science validator；同时证明 Task 不自动完成，且只替换登记的外部 Ports。场景通过 ASGI 调用 generic Host，以真实 publication coordinator 生成 immutable revision，经 mounted HMMER/Vina Drivers 和 durable Compute graph 产生两条 owner continuation；Science validator 接受显式 evidence 后 Task 仍为 `todo`，FK 检查为零。
...
- [x] 19.11 重命名或替换只验证 catalog 的 EnzymeDesign qualification 场景，更新 invariant registry、runner selection 和 requirement evidence；只有真实 runtime mount/cross-layer 场景才能声明 product Distribution proven。旧场景已改为 `wire-contract.enzymedesign-catalog` 并排除产品测试；新增 `identity-semantics.enzymedesign-product-cross-layer` 独立运行真实产品节点，closed registry 现为 28 scenario/invariant。
- [x] 19.12 在 exact clean final source 上按冻结顺序重跑 focused tests、fresh wheels、三 profile qualification、mainline、evals、strict OpenSpec 和独立 verifier，生成新的 source-bound completion evidence；clean candidate 已取得 `admission_eligible=true`、`valid=true`、零 rejection，mainline/eval/strict OpenSpec 均通过。证据封口后对最终 clean SHA 的同序复跑是本项的最终成立条件；复跑完成前保持 change active，并保持 `add-openai-compatible-delivery-surface` apply 阻塞。
