...
- [ ] 19.12 在 exact clean final source 上按冻结顺序重跑 focused tests、fresh wheels、三 profile qualification、mainline、evals、strict OpenSpec 和独立 verifier，生成新的 source-bound completion evidence；20.x 修改前的 clean candidate 与报告不能证明当前 source。复跑完成前保持 change active；后续 delivery change 必须独立验收，不能从本 change 推断可开始。
...
- [ ] 20.7 收窄当前产品资格与 metadata/documentation 声明，明确产品场景只证明 mounted graph 下的 HMMER/Vina formal cross-layer slice、Podman 无 durable terminal proof 时可长期 uncertain、真实 external qualification/cutover 未执行；完成后在最终 clean source 上重跑全部 closure evidence。
