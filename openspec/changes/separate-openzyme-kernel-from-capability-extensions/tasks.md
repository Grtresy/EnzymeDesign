...
- [x] 18.4 构建并在 fresh environments 安装全部目标 Python distributions，重算 dependency/import/content/component manifests、Adapter/Extension bundles 与 catalogs，并验证 Contracts+SPI-only、Kernel-only、Standard-only、runner-only 和 EnzymeDesign 闭包。2026-08-22 最新 source 的 37 项 component inventory 中，wheel qualifier 闭合并构建 34 个 Python distributions；Web UI 不是 wheel，`openzyme-client` 与 Host CLI 不在这五个安装 profile 的 union 内，三者由各自 package/app gate 与 mainline 独立验证。五个 fresh profile 均 pass，`network_used=false`、`external_effects_real=false`。
...
## 21. 归档前纠正组件计数、delivery identity 与资格 operation 事实

- [x] 21.1 纠正 completion evidence 中“37 component wheels”的错误口径，稳定区分 37 项 component inventory、37 个 Kernel/Plugin tools 与 wheel qualifier 实际构建的 34 个 Python distributions；保留 2026-08-21 已执行 reset 的历史 31-wheel receipt 原样，不把后验计数改写进历史证据。
- [x] 21.2 让每个 delivery surface 在 Distribution composition 中绑定 exact component、distribution name/version、build digest 与 contract digest，并由 closed parser/schema/manifest tests 拒绝缺失、未知字段和非法 digest；不得把声明闭合扩张为 CLI/UI 已随 EnzymeDesign meta-wheel 安装或部署 cutover 的证明。
- [x] 21.3 让 target qualification 的 terminal smoke observation显式携带实际 supported operations，验证其属于 qualification spec 的 closed result schema，再把同一 operations 写入 SoftwareQualificationReceipt 与 TargetCapabilityFact；空 operation、schema 外 operation 或未证明 operation 必须阻止 inventory publish。
- [x] 21.4 修正 public-interface delta 的 rename/add 语义并把 18 份 delta specs 一次性同步到 main specs；确认 requirements/scenarios 与成功预演输出一致，为 15 个新 main specs 补齐非 TBD Purpose，后续 archive 只移动 change、不重放 ADDED delta；受影响 SPI/HPC/Distribution tests、fresh wheel qualification、三 profile architecture qualification、mainline、eval、strict OpenSpec 与 independent verifier 已在最终 pre-seal candidate 上通过。该 marker 只有在随后获授权的 clean seal commit 上按 `completion-evidence.md` 整组复跑并取得 admission 后才具有 archive authority；当前 dirty candidate 不授权 archive、cutover 或 live。
