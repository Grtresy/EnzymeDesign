...
- [x] 14.6 实现 Vina local/HPC Drivers，经 explicit route + typed workload + Compute lifecycle 执行正式 docking；raw Shell 结果仅标记 exploratory。
...
- [x] 14.12 建立 generic Standard absent-vertical 测试和 EnzymeDesign product composition 正向测试，证明增加 code-review 等非生物 Plugin 无需改 Kernel。
- [x] 14.13 迁移 AOX/HMMER/structure/docking/preprocess 现有单元与 non-live integration fixtures，覆盖 capability route、typed workload、raw-shell non-formal 和既有 scientific identity/acceptance 语义；不启动 live。
...
- [x] 16.9 升级 startup read-only proof verifier，验证 installed wheels、Adapter/Extension/migration/catalog digests、Session pins、capability bindings 和 workspace backend 后才加载 runtime surfaces。
...
- [x] 17.4 实现 `enzymedesign_local_single_process_file_sqlite@1` 真实 Distribution/Plugin/Driver composition 场景，只替换登记的 LLM/provider/runner/Chrome/process external Ports。
...
- [x] 17.9 把拆分前 authority、lease/fence、effect certainty、restart/reconcile、boundedness、redaction、revision handoff 与各类 terminal 分离不变量映射到拆分后 owners。
...
- [x] 18.3 运行所有受影响package/app focused pytest、Host V3 API回归、`uv run python -m openzyme_host_api.evals`、Web UI `npm test`/build和runner wire tests，记录exact selection/result。
- [x] 18.4 构建并在 fresh environments 安装全部目标 wheels，重算 dependency/import/content/component manifests/Adapter+Extension bundles/catalogs 并验证 Contracts+SPI-only、Kernel-only、Standard-only、runner-only 和 EnzymeDesign闭包。
- [x] 18.5 运行三种 profile 的完整 non-live architecture qualification，要求所有 required scenario 执行、零 skip/xfail/undeclared effect、零 open owner/document drift 和全部 invariant satisfied。
- [x] 18.6 运行 `./scripts/check-mainline.sh` 与本change/受影响spec的strict OpenSpec validation，分开记录命令成功与任何非权威telemetry失败。
- [x] 18.7 从当前source/config/schema/manifest/wheel反向审计 `docs/OpenZyme架构设计.md`、全部相关`docs/v3/`、package/app README和operator docs，修复所有stale owner/path/command/contract/fallback描述后重跑受影响gate。
- [x] 18.8 执行 requirement-by-requirement 与 scenario-family 审计，把本 change strict validation 得到的全部 delta requirements 映射到直接 source/test/doc evidence；任何 unproven 项恢复为未完成，不使用硬编码旧计数。
- [ ] 18.9 仅在明确的后续cutover授权和quiescent环境中执行real offline `@2` migration/activation proof；本change的普通代码实施、测试或文档完成不得自行修改真实部署或启动live。
  - 2026-08-21 已获单独授权并完成旧空数据库/repository-service删除、最终EnzymeDesign fresh bootstrap、exact wheel install与独立只读startup proof；后续空数据库替换已有有效`@2` reset receipt，但最初repository-service删除发生时Store-owned executor缺失，未预先生成子项级frozen inventory/occurrence log且不能补造，故本项保持未完成。详见`operator/device-fresh-install-20260821.md`。
- [x] 18.10 在exact clean final source上生成completion evidence，确认代码、配置、migration、OpenSpec、文档、wheels和qualification同一identity后才允许archive；不得因artifacts apply-ready、旧receipt或单一绿色gate提前完成。
