...
def inspect_removal_replay(
    *,
    ...
    try:
        ...
        if not ledger["completed_at"]:
            raise RemovalAdmissionError("removal replay completion timestamp is absent")
        receipt_payload = {
            "schema": "legacy_subsystem_removal_receipt@2",
            "receipt_id": receipt_id,
            "schema_generation": ledger["schema_generation"],
            "final_schema_manifest_digest": state["manifest_digest"],
    ...
    *,
    ...
    try:
        for target in targets:
            ...
            _, path = resolve_target(target, roots)
            if not path.exists():
                if prior[0] not in {"removed", "already_absent"}:
            ...
            deleted.append(target.object_identity)
            connection.execute(
                "UPDATE legacy_removal_items SET state='removed', error_digest=NULL WHERE receipt_id=? AND object_identity=?",
                (receipt_id, target.object_identity),
            )
            refresh_error_set()
            connection.commit()
        ...
        stored_removed = sorted(
            str(row[0])
            for row in connection.execute(
                "SELECT object_identity FROM legacy_removal_items WHERE receipt_id=? AND state='removed'",
                (receipt_id,),
            ).fetchall()
        )
        stored_already_absent = sorted(
            str(row[0])
            for row in connection.execute(
                "SELECT object_identity FROM legacy_removal_items WHERE receipt_id=? AND state='already_absent'",
                (receipt_id,),
            ).fetchall()
        )
        if (
            stored_removed != sorted(deleted)
            or stored_already_absent != sorted(already_absent)
        ):
            raise RemovalAdmissionError("storage deletion disposition set differs")
        proof_identity = connection.execute(
            """
            SELECT l.schema_generation, s.manifest_digest,
                   l.historical_receipt_digest, l.database_backup_digest,
                   l.storage_backup_digest, l.quiescence_receipt_digest
            FROM legacy_removal_ledger AS l
            JOIN deployment_schema_state AS s ON s.singleton=1
            WHERE l.receipt_id=? AND l.state='incomplete'
            """,
            (receipt_id,),
        ).fetchone()
        if proof_identity is None:
            raise RemovalAdmissionError("removal completion proof identity is absent")
        completed_at = datetime.now(UTC).isoformat()
        removed_identities = set(deleted)
        receipt_payload = {
            "schema": "legacy_subsystem_removal_receipt@2",
            "receipt_id": receipt_id,
            "schema_generation": proof_identity[0],
            "final_schema_manifest_digest": proof_identity[1],
            "manifest_digest": manifest_digest,
            "historical_receipt_digest": proof_identity[2],
            "database_backup_digest": proof_identity[3],
            "storage_backup_digest": proof_identity[4],
            "quiescence_receipt_digest": proof_identity[5],
            ...
            "removed_byte_total": sum(
                item.size_bytes
                for item in targets
                if item.object_identity in removed_identities
            ),
        ...
        ledger_update = connection.execute(
            ...
            UPDATE legacy_removal_ledger SET
                ...
                removed_byte_total=?, state='complete',
        ...
        state_update = connection.execute(
            ...
            UPDATE deployment_schema_state SET
                ...
                removal_receipt_digest=?, updated_at=?
