...
The EBI HMMER route SHALL retain `bio.hmmer_search.provider:v1` while binding `provider_config:ebi_hmmer:v3`. Configured result `page_size` SHALL default to and be capped at `1000`. An EBI/Celery `RETRY` status SHALL be treated as a nonterminal state of the already accepted provider job; the adapter SHALL continue polling the same job id, SHALL NOT submit a replacement job, and SHALL remain bounded by a `3300s` polling deadline below the `3600s` sandbox deadline. A still-nonterminal job at that deadline SHALL fail as retryable `provider_timeout`; `FAILURE` and unknown statuses SHALL remain terminal fail-closed outcomes. Poll requests SHALL explicitly bind `page=1` and the configured page size, but the terminal payload SHALL be consumed only for status and non-negative `result.stats.nreported`; terminal-body hits MUST NOT become a result page. Result materialization SHALL always begin with a separate explicit page-1 request using the same page size and continue through the declared page count. Every result page SHALL contain a hits list and the same non-negative `page_count`. For a non-truncated result the materialized raw hit count MUST equal terminal `nreported`. Successful empty output SHALL require exactly terminal `nreported=0`, provider `page_count=0`, and `hits=[]` on the explicit first result request. This change MUST NOT alter `max_hits`, provider ordering, score filtering, or parsed-hit schema.

#### Scenario: Continue one accepted job through provider retry
- **WHEN** the HMMER result endpoint reports `RETRY` before later reporting `SUCCESS`
- **THEN** the adapter polls the same job id through the nonterminal state, performs no second submit, and materializes result pages only after `SUCCESS`

#### Scenario: Bound an unavailable provider retry
- **WHEN** the same job remains `RETRY` through the `3300s` polling deadline
- **THEN** the operation fails as retryable `provider_timeout` without a replacement operation, hidden replay, or partial scientific result
