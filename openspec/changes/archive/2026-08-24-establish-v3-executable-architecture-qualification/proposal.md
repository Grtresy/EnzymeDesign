...
- 明确 repository test-gate 的操作员执行能力：full qualification 需要 Starlette/AnyIO/asyncio 的本地跨线程 `socketpair`，获准的唯一公开 command 必须第一次且仅一次在具备该 IPC 的窄范围 sandbox 外环境运行；禁止 sandbox 试跑、cache 绕行、raw pytest/IPC 探针、换 output 或等价重发。平台在进程启动前拒绝能力时保持零次执行并停止，不制造 qualification 或 product failure。
