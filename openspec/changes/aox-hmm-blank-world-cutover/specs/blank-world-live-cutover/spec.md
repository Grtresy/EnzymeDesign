...
#### Scenario: Resolve the actual clean launch
- **WHEN** an operator invokes `run-live` with declared identity and prerequisite JSON
- **THEN** the launcher computes the exact-seven actual identity and safe effective-config preimage before root creation, requires field-for-field equality, and seals the same config preimage/digest into attempt launch evidence
