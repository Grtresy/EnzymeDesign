...
r63 then reached the newly ordinary scientific wake path and exposed the remaining
composition defect: Host admission succeeded, but the fresh teammate turn received
only the original task prose rather than the canonical admitted-attempt facts. The
executor repeated `attempt.create`, the authority boundary correctly rejected it,
...
- Replace the closure-only wake verifier with one bounded read-only projection over
  existing admitted-attempt, immutable-closure, and failure-observation records.
  Every resolved source remains exactly bound to signal/session/task/lane/actor;
  the projection supplies current facts without choosing the next agent action.
...
- Public/runtime contracts and tests change around `attempt.create`,
  `scientific.attempt.close`, canonical wake facts, `task.finish`, live diagnostic
  evidence, and sealed source trees.
