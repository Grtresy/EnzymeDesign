...
- 显式 `POST /v3/sessions/{session_id}/runtime/drain`
- drain 后可选 master follow-up

当前实现不能被描述成已经接入 FastAPI lifespan 的全自动后台 teammate daemon。产品主路径仍依赖显式 runtime drain 来执行 queued teammate work。
