...
- The target fixed MICU policy is exactly 500,000,000 cumulative input+output tokens; prior usage remains charged and campaign setup never resets it. An existing legacy ledger continues to enforce its stored 100M limit until the operator explicitly runs `uv run python -m openzyme_runtime_llm.live_token_ledger --migrate-legacy-fixed-policy`. Summary, reservation, and campaign startup never reinterpret it. That transaction raises only the exact legacy fixed policy, preserves all attempt rows, is idempotent at 500M, and rejects caller-selected lower limits. The cumulative ledger is read before and after every attempt.
...
Public-safety validation preserves exact sealed source bytes while rejecting
closed unsafe categories: secrets, private backend locators/URLs, traversal,
digest drift, explicit private Unix/HPC roots, Windows/UNC roots and supported
encoded forms. It does not classify every slash-prefixed program token as a
Host path; portable shebangs, application routes, custom logical selectors and
language path expressions remain source syntax when no unsafe category
matches.
...
The receipt chain MUST NOT contain `scientific-attempt-commands`, admission finalization or closure
finalization requests: those are agent/Host actors, not Codex actions. Positive product closure fixes
exactly the research, execution and reporting tasks, unique assigned agents, one owner-authored finish
per task, one source-linked report/draft, the final assistant answer and three completed statuses.
The public fault capability alone may consume the fault slot: it binds authority and the exact derived
`AOX_ref21.fasta`, flips byte zero once, and persists its claim/receipt before the unique pending MAFFT
consumer. `aox_fault_negative_state_closure@1` then proves exact
`artifact_blob_digest_mismatch`, no successful alternate, no positive report/draft and no downstream
fixed deliverable.

Production reachability qualification uses the real `ProductionCompositionFactory`, FastAPI public
routes and file-backed SQLite persistence rather than source inspection. The campaign reducer requires
one campaign and plan, exact ordinal order 1/2/3, unique attempt/session/task/lane/envelope/selection/
...
Phase 2 addresses the later r62 diagnostic without revising it. r62 ran on
`5265c7813c64880c03bef3aee15e552e4f4d1c49`, completed 13 controlled
operations, all three canonical tasks, a sealed selection and a published
report, but created zero closure request/response/closure. The earliest
scientific lifecycle failure was the executor's
`aox_cutover_close_actor_violation`; a later master
`aox_cutover_task_identity_not_ready`, empty-drain tail,
`formal_runtime_drain_exhausted` and outer
`scientific_attempt_control_missing` were downstream wrappers. Its consumed
authority, root, SQLite and decision are permanent non-acceptance evidence.

The forward contract now makes the exact attempt-task `assigned_ref` the only
closure requester and rechecks it at finalization. `scientific.attempt.close`
does not bind assistant text, report state or resident-master delivery; active
closure-response APIs and scientific-specific notification settlement are
removed while migration `035` remains historical-readable. Immutable closure
precedes generic completed task exit; its ordinary source-bound wake lets the
same assignee explicitly call `task.finish(status=completed)`. A formal
product-ready open attempt that repeats twice with replay-safe zero signals,
zero writers and no other wake source stops as
`scientific_attempt_open_no_wakeup`. Diagnostic evidence preserves raw
operation/task/report/lifecycle facts and measured MICU even without control,
...
### 2026-08-07 r74 public evidence blocker 与退休门前向修复

r74 已消费正式 authority、slot、root、session 和一次 LLM turn，但 conductor 直接使用普通 public
CLI，未从第一条 mutation 起绑定 `public-api-receipts.jsonl` 与逐次 sealed response，也未在 Host
退休前封存最终 workspace/events/handoffs。Host 本地 settlement、SQLite checkpoint/integrity 与无
active mutation writer 均通过，但退休后公开读取面已经消失，无法追溯补造 zero-attempt failure
...
或漂移时，`serve-attempt` 拒绝本次中断并保持同一 Host 可读；真实 child exit 与 authority deadline
