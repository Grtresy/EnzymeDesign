...
- `pin` 与 `preflight` 均先要求当前 clean commit 的 full architecture qualification report；qualification 通过本身只解除架构阻断，不创建 attempt，也不自动恢复 numbered campaign。current pin transaction 原子封存 credential-free launch profile，authority/preflight/Host 复用其非敏感 settings，ambient 只补 credential。`preflight` 在 atomic slot claim 和 blank-world root 后发布 source-bound conductor execution contract；若 consumed authority 的 profile/config 在 claim 前失败，则只有 current source-bound preflight-failure receipt 的纯离线 verifier/decision 可以形成零-attempt canonical NO-GO，历史 r75 不回填。Codex 经合同声明的 formal public CLI 自动绑定 receipt/response，并在 Host 退休前封存 retirement readiness。public conductor仍须先封存message、bounded drain admission、terminal status与唯一execution-task workspace read，随后才能late-bind scientific authority；executor再通过canonical lane tools和current-assignee `attempt.create`建立真实control。历史 rNN 的 frozen 状态与分类只在 cutover evidence index 维护，不在本入口复制；任何 non-live gate 都不授权下一 rNN 或 live。
