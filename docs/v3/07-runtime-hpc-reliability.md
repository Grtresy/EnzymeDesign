...
r68 首次把这个 shell 收窄为 production seam；post-r69 current schema是
`aox_supervised_host_receipt@2`，只能从 exact `aox_attempt_preflight@3` 启动固定 loopback Host child，使用独立 process group，关闭
...
campaign/plan/consumption、session/task/envelope/root、campaign-root identity与deterministic
`launch_id`，不得预造attempt/lane/admission truth。任何
...
command并由独立 status read确认 settlement，不能把 stdout、process exit或空 drain解释为业务
完成。
