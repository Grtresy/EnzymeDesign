...
```python
from openzyme_pipeline import artifacts, bio, preprocess, hpc
```
...
The pipeline cannot directly use SSH, Slurm, runner config, database connections, arbitrary network clients, or arbitrary host paths. Network database work must go through `bio.*`; HPC work must go through `hpc.*` SDK calls.
