# Vina With HPC Placement

Use `docking.vina` for AutoDock Vina docking. Use `hpc.workspace(...)`, `stage_artifact(...)`, and `fetch_outputs(...)` when the selected route runs on HPC.
...
- `receptor`: staged PDBQT artifact ref
- `ligand`: staged PDBQT artifact ref
...
- Host supervisor policy determines whether the docking job or batch requires approval; the default path is operation-preview approval before remote execution, with runtime approval only as a secondary gate
