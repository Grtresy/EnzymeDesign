...
`workspace.pending_approvals` 是浏览器的 canonical approval snapshot，SSE 事件只是低延迟 refresh trigger。当前同步 sandbox 可能先提交 approval row，待 drain 返回后才回填 `approval.requested`；Web UI 因此每五秒对当前 selected session 做一次只读 workspace reconciliation。每个 active generation 的请求必须 single-flight，并与 SSE refresh 共用 session/version freshness guard；切换 session、workspace mutation 或任一 reducer 实际写入都会 abort/失效旧 generation，旧请求的 `finally` 不得清除新的在途 generation。即使旧 GET 永不返回，新 session 仍必须能启动自己的 reconciliation。reconciliation 不得 resolve approval、推进 runtime、维护第二份真状态或用旧 response 覆盖较新的 mutation/event snapshot。
