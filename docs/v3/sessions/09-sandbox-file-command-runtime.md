...
- Host 必须在通过前序 request、workspace、layout 与 runtime 校验的每次 `sandbox.exec` 进入 source preflight 后、启动命令前调用 S08 source snapshot service，自动创建或绑定覆盖整个 `/workspace/src` 的 CODE artifact；前序校验可以更早返回自身错误，snapshot 失败时命令不启动，不能映射成普通非零退出。
- 上述进入 source preflight 的“每次”包括 `python -c`、package/signature inspection 与 diagnostics；`sandbox.exec` 不是 read-only environment inspection shortcut。agent 应优先从 controlled docs 获取 API 事实；仍需 runtime introspection 时必须先显式 author `/workspace/src` 中的 inspection source。空树固定为 `source_snapshot_empty`，并以 factual hint 说明该失败发生在 `SandboxRun` 与 process 创建前；Host 不自动生成占位源码或提供无审计 fallback。
