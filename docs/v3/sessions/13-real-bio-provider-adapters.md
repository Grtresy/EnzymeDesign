...
- `bio.uniprot_fetch` 使用 Host 托管 UniProt REST client；`provider_config:uniprot:v3` 绑定 `uniprot_primary_sequence_identity@2`。一次 SDK 调用和一次 approval/controlled operation 最多接收 `100000` 个 accession，Host 再按固定最多 `100` 个 accession 拆成 provider query。SDK `batch_size` 仍只表示每个 UniProt response page 的 `size`（上限 `100`），不是 operation/query accession 数；每个 query 独立跟随 `Link: rel=next`，并各自受 `100` 页上限约束。每个 response page 只能接受该 query exact accession slice 的 identity；跨 query swap 结构化失败。active sequence 与 exact-requested typed `Inactive/DELETED|MERGED` 对 complete requested set 作互斥分区；`DELETED` 封存非空 canonical reason，`MERGED` 封存非空、去重 replacement-target annotations，两类均封存 UniParc id、release/retrieval 和 response/record digests，固定 `identity_replaced=false`，且不跟随/抓取 replacement、不使用 sequence；unknown、`DEMERGED` 或 malformed inactive fail closed。
- `bio.hmmer_search` 使用 Host 托管 EBI HMMER REST client；`provider_config:ebi_hmmer:v3` 保留 result page size 默认/上限 `1000`，并将 EBI/Celery `RETRY` 视为同一 accepted job 的非终态。poll 显式请求 `page=1&page_size=<configured>`、在 `3300s` 内只轮询原 job id 且不重新 submit；terminal payload 只作 status/`stats.nreported` closure，result 从独立显式 page 1 开始并以同宽请求全部页，支持 sanitized provider transcript、raw hits artifact 和 parsed hits CSV artifact。
...
| `bio.uniprot_fetch` | `bio.uniprot_fetch.provider:v1` | `provider_http` | `provider_http:v1` | `provider_config:uniprot:v3` | `{"required": true}` |
| `bio.hmmer_search` | `bio.hmmer_search.provider:v1` | `provider_http` | `provider_http:v1` | `provider_config:ebi_hmmer:v3` | `{"required": true}` |
...
- provider request draft 已经建立后，即使 adapter 最终抛出 `PipelineSdkFailure`，Host 也必须通过同一 sandbox artifact boundary 登记 exact `provider_request.json`、`provider_observation.json` 和 `provider_error.json` 三件 diagnostic artifacts，再按原 canonical code/stage/retryable 语义返回并附 safe artifact refs；审批拒绝、配置缺失、`output_dir` 无效等 pre-run failure 不要求写 transcript。这三件 diagnostics 不代表 provider success，不授权 retry/replay，也不增加 AOX 的 17 件 normalized deliverable。
...
`bio.hmmer_search` under `provider_config:ebi_hmmer:v3` artifactizes at most the top 100000 hits by provider result order. Its poll URL explicitly binds `page=1` and configured `page_size` (default/maximum `1000`). `RETRY` remains nonterminal for the same provider job until success, another terminal status, or the `3300s` polling deadline; it never submits a replacement job. The terminal body is status-only evidence plus `stats.nreported`; result bytes always start at a separate explicit page 1 using the same page size. Every page must repeat one stable non-negative `page_count`. A non-truncated materialization must have raw count exactly equal to terminal `nreported`; SUCCESS empty is valid only as `nreported=0/page_count=0/hits=[]`. If provider reports more hits than the cap, `provider_observation.json`, transcript manifest and parsed outputs must record total hit count/page count when available, truncation marker and warning; truncated output is not complete cutover science. S15 may later normalize these raw provider artifacts into final `aox_hmm/*` deliverables, but S13 raw artifacts are not those final deliverables.
...
- EBI HMMER v3 polling interval is 5 seconds, total polling timeout is 3300 seconds, result `page_size` defaults to and is capped at 1000, and max artifactized hits is 100000.
...
- `ProviderRequest`
  ...
  - UniProt v3 topology：SDK 在 approval 前投影 `accession_count`、默认 `query_batch_size_cap=100` 和 `estimated_query_batch_count`；例如纠正后当前完整 HMMER 集合下 `37772` 个 accession 显示 `378` 个预计 query，但仍是一个 `ProviderRequest`、一个 controlled operation 和一次 approval。Host 对每个 query transcript 记录全局 `page`、`query_batch_index/query_batch_count`、`query_accession_start/query_accession_count/query_accessions_digest` 与 `page_in_query`；summary/observation 记录总 `page_count`、page size、per-query page cap、query batch count 与 query cap。每个 page 的 record 必须只映射到其 query slice；映射到另一个 query 的 requested identity 返回 `provider_identity_mismatch`。active records 与精确匹配 requested accession 的 typed `Inactive/DELETED|MERGED` records 必须形成 exact partition；`DELETED` 保留非空 reason，`MERGED` 保留非空、去重 replacement-target annotations，两类均保留 UniParc id、release 和 response/record digests，不含 sequence/audit、固定 `identity_replaced=false`、不跟随/抓取替换。unknown、`DEMERGED` 和 malformed inactive 仍 fail closed。next-link 只能留在 pinned UniProt HTTPS search endpoint，越界/畸形只记录 link digest并返回`provider_schema_drift`。HTTP failure 只增补 batch/page safe coordinates，不回显 URL、accession values/list 或 cursor。重复 accession 通过一次 frequency-map 扫描检测，检测阶段为 `O(n)`，再对重复 key 做稳定排序以产生确定诊断。
...
- provider request draft 已产生后，sandbox adapter 的 `PipelineSdkFailure` 也先登记 sanitized `provider_request.json` / `provider_observation.json` / `provider_error.json` exact-three diagnostic artifacts；SDK error 保留 canonical code/stage/retryable，同时附 bounded details/artifact refs，不自动 retry/replay。
...
- EBI HMMER REST 主路必须用 `database="refprot"` 覆盖 submit、显式 page-size poll、terminal-status-only 消费、从显式 page 1 开始的同宽 result pagination、跨页 `page_count` 一致、非截断 raw count=`stats.nreported`、exact empty mapping、sanitized transcript artifact 和 parsed hits output；terminal poll 内的 hits 不得当结果页。
- UniProt focused tests 必须证明：operation cap `100000` 在 HTTP 前 fail closed；`205` 个 accession 被拆为 `100/100/5` 三个 query 但只有一个 operation；纠正后 `37772` 个 accession 的 SDK resource prediction 为 `378` 个 query；`batch_size` 只改变 page size；`Link` page cap 按 query 独立计算且 link 只能指向 pinned HTTPS search endpoint；malformed/off-origin/userinfo/fragment 只泄露 digest 并 schema-drift；每页只接受 producing query slice、cross-query identity swap fail closed；active+typed inactive `DELETED|MERGED` 对 requested set 精确分区，两类 inactive 不跟随/抓取 replacement 或使用 sequence，`DEMERGED`、malformed、unknown、missing 仍失败；HTTP failure 只包含 safe batch/page 坐标；transcript query/page 坐标完整；duplicate/order checks 不做二次型 repeated scan。
...
10. UniProt provider adapter：固定 `provider_config:uniprot:v3` / `uniprot_primary_sequence_identity@2`，实现 operation accession cap `100000`、每 query 最多 `100` accession、`batch_size` page-size 语义、per-query `Link` page cap、approval 前 query-count resource estimate、query/page transcript 坐标、线性 frequency duplicate/order checks、active+typed inactive `DELETED|MERGED` exact partition、replacement-target annotation without follow/fetch、`DEMERGED` fail-closed、safe HTTP failure coordinates，以及 fields allowlist、脱敏 raw pages、FASTA/JSON parsed outputs、no-hit/schema drift 粗粒度 mapping。
11. EBI HMMER provider adapter：固定 `provider_config:ebi_hmmer:v3`，实现 JSON submit、同 job `RETRY` 非终态与 `3300s` deadline、显式 page-size poll、terminal-status-only 消费、从显式 page 1 开始的同宽 result pagination、`page_count`/`nreported` closure、top 100000 hit cap、脱敏 job/page transcript、parsed hits output、job failure 粗粒度 mapping 和 polling/pagination observation。
...
- 本轮不切换到 UniProt asynchronous ID Mapping API。当前 AOX 输入已经是 primary UniProt accessions，不需要跨 namespace mapping；改用 async submit/poll/result 会引入 durable provider job/handle、幂等提交、重启恢复、approval/operation continuation、transcript/schema 与 verifier 迁移，属于独立大架构调整。当前 v3 以一个受控 operation 内的有界 search queries 保留相同身份语义，不得把它解释为 async fallback 或无限分页。
