...
- `bio.hmmer_search` 使用 Host 托管 EBI HMMER REST client；`provider_config:ebi_hmmer:v2` 把 result page size 默认/上限固定为 `1000`。poll 显式请求 `page=1&page_size=<configured>` 但 terminal payload 只作 status/`stats.nreported` closure；result 从独立显式 page 1 开始并以同宽请求全部页，支持 sanitized provider transcript、raw hits artifact 和 parsed hits CSV artifact。
...
| `bio.hmmer_search` | `bio.hmmer_search.provider:v1` | `provider_http` | `provider_http:v1` | `provider_config:ebi_hmmer:v2` | `{"required": true}` |
...
- EBI HMMER v2 polling interval is 5 seconds, total polling timeout is 30 minutes, result `page_size` defaults to and is capped at 1000, and max artifactized hits is 100000.
...
11. EBI HMMER provider adapter：固定 `provider_config:ebi_hmmer:v2`，实现 JSON submit、显式 page-size poll、terminal-status-only 消费、从显式 page 1 开始的同宽 result pagination、`page_count`/`nreported` closure、top 100000 hit cap、脱敏 job/page transcript、parsed hits output、job failure 粗粒度 mapping 和 polling/pagination observation。
