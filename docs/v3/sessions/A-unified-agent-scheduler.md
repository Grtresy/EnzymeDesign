...
- `/runtime/drain` 只保留为 debug / operator / manual 工具，不是产品主路径。
...
- 文档检查：不应出现把 drain 描述为产品主路径、或把 teammate terminal outcome 描述为 service 直接启动 master response turn 的旧表述。
