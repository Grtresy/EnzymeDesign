...
- terminal `scientific.attempt.close` 要求 model 在同一 provider response 中同时给出非空 user-facing answer；该 text 作为 invocation 的内部 companion response，不进入 tool args。缺失时 close 在 effect 前失败。成功 close 才允许 harness 将 companion text 恰好一次持久化为 assistant conversation message，然后退休 turn；失败/rejected close 不持久化该 text。
