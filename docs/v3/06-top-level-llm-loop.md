...
- 顶层模型和 teammate 需要能力用法说明时，默认通过 `docs.search` / `docs.read` 读取受控文档库，而不是通过 skill 文档把 execution 用法塞入上下文。`WorkflowRegistry` 已解析并装载 selected manifest；`DocumentRegistry` 才是 `docs.search` / `docs.read` 的 owner。prompt 必须把 manifest path 标为 provenance-only，并只让模型从 manifest 的 `knowledge_refs` 读取知识文档；`workflow:` ref 或 `*.workflow.json` 的误投递返回 registry-owner hint，不触发相似文档/manifest fallback
...
- `workspace.runtime_state` 是请求时计算的 diagnostic projection：`agent_turn_failed`、`runtime_signal_failed`、`runtime_attention` 或 `outcome_unconsumed` / `capability_outcome_ready` 都不能自动写 task terminal state，也不在每次 drain 追加 derived warning event。task-bound、effect-known ordinary rejection 已在 exact `FailureObservation` 和下一次 ToolMessage 中结算，不建立第二个 attention/reconciliation obligation；system、unknown-effect/reconciliation、authorization 与 runtime-owned retry 边界仍以各自精确 code 保持可见。terminal capability outcome 只作为 evidence 和 wakeup source；业务 task exit 仍只能由 `task.finish` 或已文档化机械迁移完成。
